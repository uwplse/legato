...
public class CompactUnitGraph_int extends UnitGraph implements DirectedGraph<Unit> {
	private final int[] succ_ptr;
	private final int[] prev_ptr;

	private int head_ptrs;
	private int tail_ptrs;

	private final int sz;
	private final Unit[] units;

	public CompactUnitGraph_int(final DirectedGraph<Unit> in) {
		super(((UnitGraph)in).getBody());
		this.sz = in.size();
		this.succ_ptr = new int[this.sz];
		this.prev_ptr = new int[this.sz];
		...
		for(final Unit u : in) {
			...
			if(in.getHeads().contains(u)) {
				head_ptrs = (int)(head_ptrs | (((int)1) << index));
			}

			if(in.getTails().contains(u)) {
				tail_ptrs = (int)(tail_ptrs | (((int)1) << index));
			}
			...
			this.units[index] = u;
			{
				...
				for(Unit pr : in.getPredsOf(u)) {
					prev_accum = (int)(prev_accum | (((int)1) << (findOrAllocate(pr))));
				}
			...
			}
			{
				...
				for(Unit sc : in.getSuccsOf(u)) {
					succ_accum = (int)(succ_accum | (((int)1) << (findOrAllocate(sc))));
				}
