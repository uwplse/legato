...
public class CompactUnitGraph_short extends UnitGraph implements DirectedGraph<Unit> {
	private final short[] succ_ptr;
	private final short[] prev_ptr;

	private short head_ptrs;
	private short tail_ptrs;

	private final int sz;
	private final Unit[] units;

	public CompactUnitGraph_short(final DirectedGraph<Unit> in) {
		super(((UnitGraph)in).getBody());
		this.sz = in.size();
		this.succ_ptr = new short[this.sz];
		this.prev_ptr = new short[this.sz];
		...
		for(final Unit u : in) {
			...
			if(in.getHeads().contains(u)) {
				head_ptrs = (short)(head_ptrs | (((short)1) << index));
			}

			if(in.getTails().contains(u)) {
				tail_ptrs = (short)(tail_ptrs | (((short)1) << index));
			}
			...
			this.units[index] = u;
			{
				...
				for(Unit pr : in.getPredsOf(u)) {
					prev_accum = (short)(prev_accum | (((short)1) << (findOrAllocate(pr))));
				}
			...
			}
			{
				...
				for(Unit sc : in.getSuccsOf(u)) {
					succ_accum = (short)(succ_accum | (((short)1) << (findOrAllocate(sc))));
				}
